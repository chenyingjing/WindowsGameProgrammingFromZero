...
VOID CameraClass::GetTargetPosition(D3DXVECTOR3 * pLookat)
{
	if (pLookat)
	{
		*pLookat = m_vTargetPosition;
	}
}
...
VOID CameraClass::SetProjMatrix(D3DXMATRIX * pMatrix)
{
	...
	{
		D3DXMatrixPerspectiveFovLH(&m_matProj, D3DX_PI / 4.0f, (float)SCREEN_WIDTH / (float)SCREEN_HEIGHT, 1.0f, 300000.0f);
	}
...
VOID CameraClass::RotationRightVec(FLOAT fAngle)
{
	...
	m_vTargetPosition = m_vLookVector * D3DXVec3Length(&m_vCameraPosition) + m_vCameraPosition;
}
